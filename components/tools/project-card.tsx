...
export function ProjectCard({ title, body, tags, image, index, url }: ProjectCardProps) {
  ...
  const cardContent = (
    <div
      ...
      className={`flex flex-col ${isReversed ? "md:flex-row-reverse" : "md:flex-row"} gap-6 md:gap-8 p-6 rounded-xl bg-card/80 border border-gray-700/50 transition-transform duration-200 ease-out hover:scale-[1.05] ${url ? "cursor-pointer" : ""}`}
    >
